...
if __name__ == "__main__":
    ...
    vocabularies = []
    for x in vocab_idx:
        vocabularies.append(np.load(x + '.npy'))
    ...
    test_rep = []
    for i, vocab in enumerate(vocabularies):
        train_rep.append(np.load(SAVEPATH + 'bow_train_' + str(i) + '.npy'))
        test_rep.append(np.load(SAVEPATH + 'bow_test_' + str(i) + '.npy'))
        for x in [1, 3, 6]:
            start_time = time.time()
            predicted_labels = KNN_classifier(train_rep[i], train_labels, test_rep[i], x)
            end_time = time.time()
    ...
    # Your code below
    for svm_lambda in [1, 0.1, 10]:
        for x in range(0, len(train_rep)):
            start_time = time.time()
            predicted_labels = SVM_classifier(train_rep[x], train_labels, test_rep[x], True, svm_lambda)
            end_time = time.time()
    ...
    for svm_lambda in [1, 0.1, 10]:
        for x in range(0, len(train_rep)):
            start_time = time.time()
            predicted_labels = SVM_classifier(train_rep[x], train_labels, test_rep[x], False, svm_lambda)
            end_time = time.time()
            lin_accuracies.append(reportAccuracy(test_labels, predicted_labels))
            lin_runtimes.append(end_time - start_time)
